...
VPIAprilTagFamily ToVPIAprilTagFamily(const std::string& family)
{
  ...
  if (it == g_str_to_vpi_apriltag_family.end()) {
    return VPI_APRILTAG_INVALID;
  }
  return it->second;
}
...
{
  auto it = std::find_if(
      std::begin(g_str_to_vpi_apriltag_family),
      std::end(g_str_to_vpi_apriltag_family),
      [family](auto&& p) { return p.second == family; });
  if (it == std::end(g_str_to_vpi_apriltag_family)) {
    return "";
  }
  return {it->first};
}

std::unordered_map<std::string, VPIImageFormat>
    g_encoding_str_to_vpi_image_format(
...
VPIImageFormat ToVPIImageFormatFromROSEncoding(const std::string& encoding)
{
  ...
  if (it == g_encoding_str_to_vpi_image_format.end()) {
    return VPI_IMAGE_FORMAT_INVALID;
  }
  return it->second;
}
...
struct AprilTagNode::AprilTagImpl
{
  ...
  virtual std::unordered_set<VPIAprilTagFamily>
  SupportedTagFamilies() const = 0;
...
struct AprilTagNode::VPIAprilTagImpl : AprilTagNode::AprilTagImpl
{
  ...
  std::vector<uint16_t> tag_id_filter_buffer_{};
  ...
  std::unordered_set<VPIAprilTagFamily> SupportedTagFamilies() const override
  {
    ...
    std::transform(
        g_str_to_vpi_apriltag_family.begin(),
        g_str_to_vpi_apriltag_family.end(),
        std::inserter(tag_families, tag_families.end()),
        [](auto&& key_value_pair) { return key_value_pair.second; });
    return tag_families;
  }

  void makeTagIDFilter(
      VPIAprilTagDecodeParams* params,
      const std::vector<uint16_t>& tag_id_filter)
  {
    tag_id_filter_buffer_.clear();

    if (tag_id_filter.empty()) {
      params->tagIdFilter = nullptr;
      params->tagIdFilterSize = 0;
      return;
    }

    tag_id_filter_buffer_ = tag_id_filter;

    params->tagIdFilter = tag_id_filter_buffer_.data();
    params->tagIdFilterSize
        = static_cast<int32_t>(tag_id_filter_buffer_.size());
  }
  ...
  {
    AprilTagNode::AprilTagImpl::Initialize(
        node, nitros_image_view, camera_info);

    try {
      CHECK_STATUS(vpiStreamCreate(
          node.backends_ | VPI_BACKEND_CPU | VPI_BACKEND_CUDA, &stream_));
    ...
    makeTagIDFilter(&params_, node.tag_id_filter_);
    ...
    VPICameraIntrinsic cam_intrinsics = {{fx, sk, cx}, {0, fy, cy}};
    std::memcpy(&cam_intrinsics_, &cam_intrinsics, sizeof(VPICameraIntrinsic));

    // Detector
    CHECK_STATUS(vpiCreateAprilTagDetector(
        node.backends_,
        camera_info->width,
        camera_info->height,
        &params_,
    ...
    data->buffer.pitch.format
        = ToVPIImageFormatFromROSEncoding(nitros_image_view.GetEncoding());
    data->buffer.pitch.numPlanes = 1;
    data->buffer.pitch.planes[0].data = const_cast<void*>(
        reinterpret_cast<const void*>(nitros_image_view.GetGpuData()));
    ...
    CHECK_STATUS(
        vpiImageCreateWrapper(data, nullptr, VPI_BACKEND_CUDA, &input_image_));

    RCLCPP_INFO(
        node.get_logger(),
        "Initialized with input image (%dx%d), encoding=%s, pitch=%d",
      ...
      const sensor_msgs::msg::CameraInfo::ConstSharedPtr& camera_info) override
  {
    ...
    input_image_data_.buffer.pitch.planes[0].data = const_cast<void*>(
        reinterpret_cast<const void*>(nitros_image_view.GetGpuData()));
    ...
    CHECK_STATUS(vpiArrayCreate(
        node.max_tags_,
        VPI_ARRAY_TYPE_APRILTAG_DETECTION,
        0,
        &detections_array));
    CHECK_STATUS(vpiSubmitAprilTagDetector(
        stream_,
        node.backends_,
        detector_,
        node.max_tags_,
    ...
    CHECK_STATUS(vpiArrayCreate(
        node.max_tags_, VPI_ARRAY_TYPE_POSE, 0, &detections_pose_array));
    CHECK_STATUS(vpiSubmitAprilTagPoseEstimation(
        stream_,
        VPI_BACKEND_CPU,
        detections_array,
        cam_intrinsics_,
    ...
    CHECK_STATUS(vpiArrayLockData(
        detections_array,
        VPI_LOCK_READ,
        VPI_ARRAY_BUFFER_HOST_AOS,
        &detections_data));
    VPIAprilTagDetection* detections = reinterpret_cast<VPIAprilTagDetection*>(
        detections_data.buffer.aos.data);
    ...
    VPIPose* poses
        = reinterpret_cast<VPIPose*>(detections_pose_data.buffer.aos.data);
    ...
    for (int32_t i = 0; i < num_detections; i++) {
      ...
      if (node.tag_names_.find(detection.id) != node.tag_names_.end()) {
        tf.child_frame_id = std::string(node.tag_names_.at(detection.id));
      } else {
        tf.child_frame_id
...
struct AprilTagNode::CUAprilTagImpl : AprilTagNode::AprilTagImpl
{
  ...
  {
    switch (family) {
    case VPI_APRILTAG_36H11:
      return cuAprilTagsFamily::NVAT_TAG36H11;
    default:
      return cuAprilTagsFamily::NVAT_ENUM_SIZE;
    }
  }
      ...
      const sensor_msgs::msg::CameraInfo::ConstSharedPtr& camera_info) override
  {
    ...
    const int error = nvCreateAprilTagsDetector(
        ...
        camera_info->height,
        node.tile_size_,
        ToCuAprilTagsFamily(tag_family_),
        &cam_intrinsics_,
        node.size_);
    if (error != 0) {
      ...
      const sensor_msgs::msg::CameraInfo::ConstSharedPtr& camera_info) override
  {
    ...
    if (nitros_image_view.GetEncoding() != "rgb8"
        && nitros_image_view.GetEncoding() != "bgr8") {
      RCLCPP_ERROR(
          node.get_logger(),
    ...
    input_image.dev_ptr = const_cast<uchar3*>(
        reinterpret_cast<const uchar3*>(nitros_image_view.GetGpuData()));
    ...
    if (error != 0) {
      RCLCPP_ERROR(
          node.get_logger(),
          "Failed to run AprilTags detector (error code %d)",
          error);
      return;
    }
    ...
    for (uint32_t i = 0; i < num_detections; i++) {
      ...
      const float intercept_1
          = detection.corners[0].y - (slope_1 * detection.corners[0].x);
      const float intercept_2
          = detection.corners[3].y - (slope_2 * detection.corners[3].x);
      msg_detection.center.x
      ...
      // Pose
      msg_detection.pose.header = camera_info->header;
      msg_detection.pose.pose.pose.position.x = tf.transform.translation.x;
      msg_detection.pose.pose.pose.position.y = tf.transform.translation.y;
      msg_detection.pose.pose.pose.position.z = tf.transform.translation.z;
      msg_detection.pose.pose.pose.orientation = tf.transform.rotation;
...
AprilTagNode::AprilTagNode(const rclcpp::NodeOptions& options) :
    ...
    backends_{::isaac_ros::common::DeclareVPIBackendParameter(
        this, VPI_BACKEND_CUDA)},
    image_sub_{},
    camera_info_sub_{},
        ...
        "tag_detections", rclcpp::QoS(1))}
{
  ...
  if (supported_tag_families.find(ToVPIAprilTagFamily(tag_family_))
    ...
    os << "Tag family not supported by specified backend: '" << tag_family_
       << "'\n";
    os << "'tag_family' parameter must be one of:\n";
    for (const auto& entry : supported_tag_families) {
      os << ToString(entry) << "\n";
    }
  ...
  std::vector<int64_t> tag_ids_raw = declare_parameter<std::vector<int64_t>>(
      "tag_id_filter", std::vector<int64_t>{});
  tag_id_filter_.reserve(tag_ids_raw.size());
  for (int64_t id : tag_ids_raw) {
    if (id < 0 || id > std::numeric_limits<uint16_t>::max()) {
      throw std::runtime_error(
          "tag_id_filter value out of range: " + std::to_string(id));
    }
    tag_id_filter_.push_back(static_cast<uint16_t>(id));
  }

  std::vector<std::string> tags_param
      = declare_parameter<std::vector<std::string>>(
          "standalone_tags", std::vector<std::string>{});

  tag_sizes_.clear();
  tag_names_.clear();

  for (const auto& tag_entry : tags_param) {
    std::istringstream iss(tag_entry);
    std::string id_str, size_str, name;

    if (!std::getline(iss, id_str, ':') || !std::getline(iss, size_str, ':')) {
      RCLCPP_WARN(get_logger(), "Invalid tag format: %s", tag_entry.c_str());
      continue;
    }

    std::getline(iss, name); // optional

    try {
      int64_t id = std::stoll(id_str);
      double size = std::stod(size_str);

      if (id < 0 || id > std::numeric_limits<uint16_t>::max() || size <= 0.0) {
        RCLCPP_WARN(
            get_logger(),
            "Skipping tag with invalid id/size: %s",
            tag_entry.c_str());
        continue;
      }

      tag_sizes_[static_cast<uint16_t>(id)] = size;
      if (!name.empty()) {
        tag_names_[static_cast<uint16_t>(id)] = name;
      }
    } catch (const std::exception& e) {
      RCLCPP_WARN(
          get_logger(),
          "Exception parsing standalone_tag '%s': %s",
          tag_entry.c_str(),
          e.what());
    }
  }

  camera_image_sync_.registerCallback(std::bind(
      &AprilTagNode::CameraImageCallback,
      this,
      std::placeholders::_1,
      std::placeholders::_2));
  ...
  printParams();
  RCLCPP_INFO(get_logger(), "AprilTagNode initialized.");
}
...
void AprilTagNode::CameraImageCallback(
    ...
    const sensor_msgs::msg::CameraInfo::ConstSharedPtr& camera_info)
{
  auto nitros_image_view
      = nvidia::isaac_ros::nitros::NitrosImageView(*nitros_image);
...
void AprilTagNode::printParams()
{
  RCLCPP_INFO(get_logger(), "=== AprilTagNode Configuration ===");
  RCLCPP_INFO(get_logger(), "Tag family      : %s", tag_family_.c_str());
  RCLCPP_INFO(get_logger(), "Max tags        : %d", max_tags_);
  RCLCPP_INFO(get_logger(), "Default size    : %.3f", size_);
  RCLCPP_INFO(get_logger(), "Tile size       : %u", tile_size_);
  RCLCPP_INFO(get_logger(), "CUDA backends   : %d", backends_);

  if (!tag_id_filter_.empty()) {
    std::ostringstream oss;
    oss << "Filtered tag IDs: [";
    for (size_t i = 0; i < tag_id_filter_.size(); ++i) {
      oss << tag_id_filter_[i];
      if (i + 1 < tag_id_filter_.size()) {
        oss << ", ";
      }
    }
    oss << "]";
    ...
    RCLCPP_INFO(get_logger(), "Filtered tag IDs: none");
  }

  if (!tag_sizes_.empty()) {
    RCLCPP_INFO(get_logger(), "Configured standalone tags:");
    for (const auto& [id, size] : tag_sizes_) {
      auto it = tag_names_.find(id);
      if (it != tag_names_.end()) {
        ...
        RCLCPP_INFO(get_logger(), " - ID %d: size=%.3f", id, size);
      }
    }
  } else {
    RCLCPP_INFO(get_logger(), "No standalone tags configured.");
  }
